  ...
  return (
    <PostLayout slug="toekomstdroom" title="Toekomstdroom" date="2 okt 2024" readTime="3 minuten" img={null} likes={0} comments={0}>
      {[
        ...
        <div key={i} className="reveal glow-card" style={{ padding: "36px", borderRadius: "24px", marginBottom: "20px", transitionDelay: `${i * 100}ms` }}>
          <div style={{ display: "flex", gap: "16px", alignItems: "flex-start" }}>
            <div style={{ width: "40px", height: "40px", flexShrink: 0, background: "linear-gradient(135deg, #7c3aed, #4f46e5)", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", fontWeight: 800, color: "white" }}>{s.num}</div>
            <div>
              <h3 style={{ marginBottom: "12px", fontSize: "1rem" }}>{s.title}</h3>
              <p style={{ fontSize: "14px" }}>{s.text}</p>
            </div>
          </div>
        </div>
      ))}
